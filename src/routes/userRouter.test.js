...
test('GET /api/user/me returns authenticated user', async () => {
  ...
  expect(res.status).toBe(200);
  expect(res.body).toMatchObject({ id: diner.id, email: diner.email, name: diner.name });
});
...
test('PUT /api/user/:id blocks non-admin updating other user', async () => {
  ...
  const res = await request(app)
    ...
    .set('Authorization', `Bearer ${diner.token}`)
    .send({ name: `hack-${randomName()}`, email: other.email, password: other.password });
...
test('PUT /api/user/:id allows admin updating other user', async () => {
  ...
  const res = await request(app)
    ...
    .send({ name: `adminedit-${randomName()}`, email: other.email, password: other.password });
